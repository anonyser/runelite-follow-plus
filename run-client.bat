...
if not "%~1"=="hi" (
    start "Follow Plus (dev)" /HIGH "%~f0" hi
    exit /b
)
...
set "SRC=\\wsl.localhost\Ubuntu\home\user\runelite-follow-plus"
set "DST=C:\Users\markl\runelite-follow-plus"

echo Syncing latest plugin source from WSL...
robocopy "%SRC%" "%DST%" /MIR /XD .git build .gradle /NFL /NDL /NJH /NJS /NP >nul

cd /d "%DST%"
echo Launching RuneLite at HIGH priority (8GB max heap) with Follow Plus...
...
echo Client closed. Dev logs: C:\Users\markl\.runelite\followplus-dev.log
pause
